"""
EfficientNetV2 model implementation for SatRain dataset.
...
class EfficientNetV2Config:
    """Configuration for EfficientNetV2 model."""
    ...
    @classmethod
    def from_dict(cls, config_dict: Dict[str, Any]) -> "EfficientNetV2Config":
        """Create EfficientNetV2Config from dictionary."""
        return cls(**config_dict)

    @classmethod
    def from_toml_file(cls, toml_path: Union[str, Path]) -> "EfficientNetV2Config":
        """Create EfficientNetV2Config from TOML file."""
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert EfficientNetV2Config to dictionary representation."""
        return asdict(self)
...
class UNetDecoder(nn.Module):
    ...
    def __init__(self, encoder_channels: int, decoder_channels: list, n_outputs: int = 1):
        ...
        self.decode1 = self._decode_block(decoder_channels[2] + decoder_channels[3], decoder_channels[3])
    ...
    def forward(self, x, skip_connections):
        ...
        Args:
            ...
            skip_connections: List of skip connections [stride_16, stride_8, stride_4, stride_2]
        """
        ...
        # Decoder stage 1: stride 4 -> stride 2
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False)
...
class EfficientNetV2Regression(nn.Module):
    """EfficientNetV2 with UNet-style decoder for regression with spatial output."""
    def __init__(self, base_model, n_channels: int, n_outputs: int = 1):
        super().__init__()
        
        # Keep only the features part of the base model
    ...
    def _analyze_and_extract_skip_info(self, n_channels: int):
        ...
        # Extract skip connections in order: stride 16, 8, 4, 2
        # These match decoder upsampling stages
        self.skip_indices = []
        self.skip_channels = []
        self.skip_strides = []
        
        for target_stride in [16, 8, 4, 2]:
...
def create_efficient_net_v2(n_channels: int, n_outputs: int = 1, 
    ...
    """
    Create an EfficientNetV2 model with UNet decoder for regression with spatial output.
    ...
    Returns:
        Modified EfficientNetV2 model with UNet decoder
    ...
    # Create base model
    if model_size == 'small':
        base_model = efficientnet_v2_s(weights='IMAGENET1K_V1' if pretrained else None)
    elif model_size == 'medium':
        base_model = efficientnet_v2_m(weights='IMAGENET1K_V1' if pretrained else None)
    elif model_size == 'large':
        base_model = efficientnet_v2_l(weights='IMAGENET1K_V1' if pretrained else None)
    else:
        raise ValueError(f"model_size must be 'small', 'medium', or 'large', got {model_size}")

    # Create regression model with UNet decoder
    model = EfficientNetV2Regression(
        base_model=base_model,
        n_channels=n_channels,
        n_outputs=n_outputs
    )
    
    logger.info(f"Created EfficientNetV2 model (size={model_size}, n_channels={n_channels}, n_outputs={n_outputs})")
    return model